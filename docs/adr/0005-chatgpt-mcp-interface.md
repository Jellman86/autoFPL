...
2. ChatGPT owns its model session. autoFPL does not receive or store OpenAI/ChatGPT OAuth tokens and does not exchange a ChatGPT subscription for general API access. It cannot observe or enforce ChatGPT's model-turn, token, retry or provider-spend budgets.
3. Hermes owns its selected model provider and credentials. autoFPL does not receive them when Hermes is acting as an MCP client and cannot observe or enforce Hermes' upstream model-loop budget.
4. The initial MCP surface exposes read-only advisory tools only. From the user and external-system perspective, tools may retrieve versioned squad state, forecasts, simulations, optimiser proposals, uncertainty and evidence or request a bounded idempotent analytical run. Such a request may append an immutable run artefact/audit record but cannot mutate squad/business state, act on an FPL account or mark a proposal human-approved.
...
6. The MCP adapter calls the same deterministic application services used by the dashboard. Under [ADR-0007](0007-evidence-grounded-ai-decision-orchestrator.md), the model is an untrusted evidence-grounded decision orchestrator: it may retrieve evidence, request additional scenarios, compare validated candidates and explain a preferred advisory synthesis, but it is never the source of forecast, constraint, optimisation, authoritative-memory or approval truth. The baseline MCP path does not persist the client's generated synthesis as an autoFPL proposal.
