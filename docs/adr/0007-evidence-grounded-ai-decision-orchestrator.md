...
7. SQLite and versioned source/analytical artefacts are **authoritative memory** for structured state and audit. OpenViking or another retrieval store may hold unstructured research, summaries and contextual memory, but retrieved text remains non-authoritative evidence until linked to a source or record.
