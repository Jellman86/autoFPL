...
- **Initial lawful source admissions now have an executable provenance contract.** A fail-closed Draft-07 schema binds manual input and repository-authored synthetic fixtures to their immutable admission evidence, exact purpose, rights, UTC timing, content hash and correction identity; checked-in examples are enforced by JSON Schema and a semantic governance policy without adding runtime ingestion or persistence.
