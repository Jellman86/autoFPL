...
The executable backend validates decision-snapshot metadata and deterministic manual squad and starting-XI feasibility. Metadata schema version `1.0` is supported, and source type is restricted to `manual` or `synthetic`; unsupported values fail with stable domain error codes. Squad and lineup inputs remain user-supplied and do not call FPL or perform account actions.
...
- `AutoFpl.Api` exposes liveness, readiness and fail-closed metadata, squad and lineup validation over HTTP;
