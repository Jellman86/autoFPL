...
internal static class DatabaseMigrations
{
    ...
    [
        ...
        new(
            33,
            "best-supported-opening-squad-artifact",
            """
            PRAGMA defer_foreign_keys = ON;

            CREATE TABLE selected_opening_squad_shadow_artifacts_v33 (
                selected_opening_squad_artifact_id INTEGER PRIMARY KEY,
                schema_version TEXT NOT NULL CHECK (schema_version = '1.0'),
                artifact_type TEXT NOT NULL
                    CHECK (
            ...
            INSERT INTO selected_opening_squad_shadow_artifacts_v33
            SELECT * FROM selected_opening_squad_shadow_artifacts;

            DROP TRIGGER selected_opening_squad_shadow_immutable;
            DROP TRIGGER selected_opening_squad_shadow_no_delete;
            DROP TABLE selected_opening_squad_shadow_artifacts;

            ALTER TABLE selected_opening_squad_shadow_artifacts_v33
                RENAME TO selected_opening_squad_shadow_artifacts;
        ...
        new(
            34,
            "historical-fbref-championship-playing-time",
            """
            PRAGMA defer_foreign_keys = ON;

            CREATE TABLE research_source_snapshots_v34 (
                snapshot_id INTEGER PRIMARY KEY,
                schema_version TEXT NOT NULL CHECK (schema_version = '1.0'),
                status TEXT NOT NULL CHECK (status = 'shadow-only'),
                source_key TEXT NOT NULL
            ...
            INSERT INTO research_source_snapshots_v34 (
                snapshot_id,
                schema_version,
                status,
                source_key,
        ...
        new(
            37,
            "public-market-projection-source",
            """
            PRAGMA defer_foreign_keys = ON;

            CREATE TABLE research_source_snapshots_v37 (
                snapshot_id INTEGER PRIMARY KEY,
                schema_version TEXT NOT NULL CHECK (schema_version = '1.0'),
                status TEXT NOT NULL CHECK (status = 'shadow-only'),
                source_key TEXT NOT NULL
            ...
            INSERT INTO research_source_snapshots_v37 (
                snapshot_id,
                schema_version,
                status,
                source_key,
            ...
            DROP TABLE research_source_snapshots;
            ALTER TABLE research_source_snapshots_v37
                RENAME TO research_source_snapshots;
        ...
        new(
            38,
            "public-projection-opening-squad-artifact",
            """
            CREATE TABLE public_projection_opening_squad_artifacts (
                ...
                created_at_utc TEXT NOT NULL,
                UNIQUE (official_capture_id, source_snapshot_id)
            );

            CREATE INDEX public_projection_opening_squad_latest_idx
                ON public_projection_opening_squad_artifacts (
                    official_capture_id,
                    source_snapshot_id DESC,
                    artifact_id DESC
                );

            CREATE TRIGGER public_projection_opening_squad_immutable
            BEFORE UPDATE ON public_projection_opening_squad_artifacts
            BEGIN
                SELECT RAISE(
                    ABORT,
                    'public projection squad artifacts are immutable'
                );
            END;

            CREATE TRIGGER public_projection_opening_squad_no_delete
            BEFORE DELETE ON public_projection_opening_squad_artifacts
            BEGIN
                SELECT RAISE(
                    ABORT,
