...
public sealed class SelectedOpeningSquadInboxPoller : BackgroundService
{
    public const string FilePrefix = "selected-opening-squad-capture-";
    ...
    private readonly SelectedOpeningSquadShadowImporter _importer;
    private readonly ShadowForecastInboxOptions _options;
    private readonly TimeProvider _timeProvider;

    public SelectedOpeningSquadInboxPoller(
        SelectedOpeningSquadShadowImporter importer,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        TimeSpan interval = _options.Interval
            ?? throw new InvalidOperationException(
                "The selected-squad inbox poller requires an interval.");
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // Serving remains available while the private handoff is
