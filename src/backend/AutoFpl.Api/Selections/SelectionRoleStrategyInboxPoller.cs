...
public sealed class SelectionRoleStrategyInboxPoller : BackgroundService
{
    public const string FilePrefix = "selection-role-strategies-capture-";
    ...
    private readonly SelectionRoleStrategyShadowImporter _importer;
    private readonly ShadowForecastInboxOptions _options;
    private readonly TimeProvider _timeProvider;

    public SelectionRoleStrategyInboxPoller(
        SelectionRoleStrategyShadowImporter importer,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        TimeSpan interval = _options.Interval
            ?? throw new InvalidOperationException(
                "The strategy inbox poller cannot run without an interval.");
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // Serving remains available while the private inbox is
